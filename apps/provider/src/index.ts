...
const prices: Record<string, string> = { validate_project_strategy: "10000", audit_submission: "50000" };
const network = () => process.env.HEDERA_NETWORK ?? "hedera:testnet";
const asset = () => process.env.HEDERA_USDC_TOKEN_ID ?? "0.0.429274";
const payTo = () => {
  const value = process.env.HEDERA_RECIPIENT_ACCOUNT_ID;
  if (!value && process.env.DEMO_MODE !== "true") throw new Error("HEDERA_RECIPIENT_ACCOUNT_ID is required when DEMO_MODE is not true.");
...
export function createProvider() {
  const app = new Hono();
  app.use("*", cors({ origin: (process.env.CORS_ORIGIN ?? "http://localhost:3000").split(","), allowHeaders: ["Content-Type", "PAYMENT-SIGNATURE", "Idempotency-Key"], exposeHeaders: ["PAYMENT-REQUIRED", "PAYMENT-RESPONSE"], allowMethods: ["POST", "OPTIONS"] }));
  app.get("/health", (c) => c.json({ ok: true, service: "hackrails-provider", scheme: "exact", network: network(), facilitatorUrl: facilitatorUrl(), demoMode: process.env.DEMO_MODE === "true" }));
  app.use("/tools/:tool", async (c, next) => {
    if (!(c.req.param("tool") in prices)) return c.json({ error: "Unknown premium tool." }, 404);
    await next();
  });
  const resourceServer = new x402ResourceServer(new HTTPFacilitatorClient({ url: facilitatorUrl() })).register("hedera:*", new ExactHederaScheme({ defaultAssets: { [network()]: { asset: asset(), decimals: 6 } } }));
...
if (process.env.NODE_ENV !== "test") serve({ fetch: createProvider().fetch, port: Number(process.env.PORT ?? 4002) }, (info) => console.log(`HackRails provider listening on ${info.port}`));
